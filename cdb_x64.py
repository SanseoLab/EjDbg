import winpexpect, sys, io, re, time
from tkinter import *
...
global newrip			# 명령어 실행 후 변경된 RIP 레지스터.
...
oldregister = { 'rax' : '', 'rbx' : '', 'rcx' : '', 'rdx' : '', 'rsi' : '', 
				'rdi' : '', 'rip' : '', 'rsp' : '', 'rbp' : '', 'r8' : '', 
				'r9' : '', 'r10' : '', 'r11' : '', 'r12' : '', 'r13' : '', 
				'r14' : '', 'r15' : '', 'efl' : '' }
...
"""
Windbg x64 버전의 특징 상 주소에 00007ffa`4f182e9c처럼 Grave(`) 문자가 들어가 있다.
이 문자를 제거해준다.
"""
@memoize
...
def findiat():
	...
	for searchRes in res.splitlines():
		...
		if int(extractGrave(firstline[:17]), 16) + 6 != int(extractGrave(secondline[:17]), 16):
			firstline = secondline
		else:
			indirectStart = int(extractGrave(firstline[:17]), 16)
			break
...
def asmview(preins, isinternal, adres):
	...
	ins = "ub " + newrip + " l9; .printf \"<>\";" + "u " + newrip + " l12"
	child.sendline(ins)
	child.expect('0:000> ')
	usResult = newstdout.getvalue()

	if usResult.find('Unable to') == -1:

		ubHalf = usResult.find('<>')
		# ubResult는 ub 명령어의 결과이다.
		ubResult = usResult[:ubHalf-1]
		# uResult는 u 명령어의 결과이다.
		uResult = usResult[ubHalf+2:]

		# firstAddress는 ,vs 명령어에서 사용될 때 다음 페이지를 구분하기 위해 이용된다.
		resultLN = ubResult.find('\n')
	
		firstAddress = ubResult[resultLN+1:resultLN+18]
	...
	# 각 라인을 읽은 후에 다음을 수행한다.
		...
		i = i+1
		addresses[i] = extractGrave(line[:17])
...
def outview():
	...
	# 명령어들을 최대한 붙여쓰는 이유는 winpexpect를 통한 sendline, expect 사용 시마다 오버헤드가 크기 때문이다.
	child.sendline('r;    .printf "<>1";    .printf "rax ";da @rax; .printf "rbx "; da @rbx; .printf "rcx "; da @rcx; .printf "rdx "; da @rdx; .printf "rsi "; da @rsi; .printf "rdi "; da @rdi; .printf "r8 "; da @r8; .printf "r9 "; da @r9; .printf "r10 "; da @r10; .printf "r11 "; da @r11; .printf "r12 "; da @r12; .printf "r13 "; da @r13; .printf "r14 "; da @r14; .printf "r15 "; da @r15;  .printf "<>2";    dqs @rsp l12;  .printf "<>3";  dqa @rsp l12')
	child.expect('0:000> ')
	...
	newregister = { 'rax' : rResult[4:20], 'rbx' : rResult[25:42], 'rcx' : rResult[46:62], 'rdx' : rResult[67:83], 'rsi' : rResult[88:104], 
					...
					'r14' : rResult[319:335], 'r15' : rResult[340:356], 'efl' : rResult[465:473] }
	...
	for key in oldregister:
		if oldregister[key] != newregister[key]:
			if key == 'rax':
				...
				panelReg2.tag_add("efl", "9.6", "9.14")
