...
oldregister = { 'eax' : '', 'ecx' : '', 'edx' : '', 'ebx' : '', 'esp' : '', 'ebp' : '',
		'esi' : '', 'edi' : '', 'eip' : '', 'eflags' : ''}
...
"""
디스어셈블리 패널을 위한 함수. 
두 곳에서 사용되는데 한나는 s, ni 같은 일반 제어 명령들과 ,v 자체 명령어이다. 이것은 IP 레지스터가 바뀌었다는 가정 하에 실행된다.
...
def asmview(isInternal, Adres):
	...
	if isInternal == 0:
		newAddress = hex(int(Adres, 16)-25)
	elif isInternal == 1:
		newAddress = hex(int(oldregister['eip'],16)-25)
...
def outview():
	...
	for line in rResult.splitlines():
		...
		if regName == 'eax' or regName == 'edx' or regName == 'esp' or regName == 'esi' or regName == 'eip':
			view = view + regName + '\t' + newregister[regName] + '\n'
		elif regName == 'ecx' or regName == 'ebx' or regName == 'ebp' or regName == 'edi' or regName == 'eflags':
	...
	for key in oldregister:
		if oldregister[key] != newregister[key]:
			if key == 'eax':
				panelReg1.tag_add("eax", "1.4", "1.14")
				panelReg1.tag_config("eax", foreground="blue")
			elif key == 'ecx':
				panelReg2.tag_add("ecx", "1.4", "1.14")
				panelReg2.tag_config("ecx", foreground="blue")
			elif key == 'edx':
				panelReg1.tag_add("edx", "2.4", "2.14")
				panelReg1.tag_config("edx", foreground="blue")
			elif key == 'ebx':
				panelReg2.tag_add("ebx", "2.4", "2.14")
				panelReg2.tag_config("ebx", foreground="blue")
			elif key == 'esp':
				panelReg1.tag_add("esp", "3.4", "3.14")
				panelReg1.tag_config("esp", foreground="blue")
			elif key == 'ebp':
				panelReg2.tag_add("ebp", "3.4", "3.14")
				panelReg2.tag_config("ebp", foreground="blue")
			elif key == 'esi':
				panelReg1.tag_add("esi", "4.4", "4.14")
				panelReg1.tag_config("esi", foreground="blue")
			elif key == 'edi':
				panelReg2.tag_add("edi", "4.4", "4.14")
				panelReg2.tag_config("edi", foreground="blue")
			elif key == 'eip':
				panelReg1.tag_add("eip", "5.4", "5.14")
				panelReg1.tag_config("eip", foreground="blue")
			elif key == 'eflags':
				panelReg2.tag_add("eflags", "5.6", "5.15")
